...
(one-step or multistep) and chord diagram displaying the inter-relationships 
between the land uses.

## São Lourenço river Basin Case Study

For the illustrative case study, we're going to use the map (shape file) from the 4th edition of the [Monitoring 
...
The shapefile was transformed into rasters and then saved as a 5-layers `RasterStack`, 
...
layer (`RasterBrick`, `RasterStack` or a path to folder containing the rasters). The name
of the rasters must be in the format ("some-text" `+` "underscore" `+` "the-year") like "landscape_2019".
...
value and a color to be associate with (important for the plots). The values in class
...
An ``r class(testSL[[2]])`` object contains two slots: the first one with the analysed level and the
second a table of interval result.
An ``r class(testSL[[4]])`` object contains four slots: the first with the analysed level, the
second the colors associate with the data with the legends as name's attribute, the third slot contain
a table of the category level result _(gain or loss)_  and the fourth slot contain a table 
saving a stationarity test.
An ``r class(testSL[[6]])`` object contains four slots: the first with the analysed level, the
second the colors associate with the data with the legends as name's attribute, the third slot contain
a table of the transition level result _(gain n or loss m)_ and the fourth slot contains a table 
saving a stationarity test.

> @Aldwaik2012 considers a stationary case only whether the intensities for all time 
intervals reside on one side of the uniform intensity.


```{r}
# showing the object from the intensity analysis for our illustrative case
testSL
...
Visualizations of the intensity analysis result is obtaining from the plot(intensityObject) 
