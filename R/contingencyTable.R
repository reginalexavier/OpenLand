...
#'         \item The colors have to in the same order as the values in the
#'          \code{categoryValue} column. Colors can be given by the color name
...
contingencyTable <-
  function(input_raster, pixelresolution = 30) {
    ...
    table_cross <- function(x, y) {
      ...
      contengency %>%
        dplyr::mutate(
          Year_from = colnames(contengency)[1],
    ...
    if (raster::nlayers(rList) == 2) {
      ...
      table_multi <- Reduce(
        ...
        mapply(
          function(x, y) {
            table_cross(x, y)
          }, rList_multi[1:(length(rList_multi) - 1)],
          rList_multi[2:length(rList_multi)],
          SIMPLIFY = FALSE
        )
      )
    }
    ...
    lulctable <-
      lapply(lulc, function(x) {
        x %>%
          ...
          dplyr::mutate(
            yearFrom = as.integer(yearFrom), yearTo = as.integer(yearTo),
    ...
    areaTotal <-
      ...
      dplyr::summarise(area_km2 = sum(km2), QtPixel = sum(QtPixel))
