...
intensityAnalysis <-
  function(dataset, class_n, class_m, area_km2 = TRUE) {
    ...
    if (isTRUE(area_km2)) {
      ...
      # EQ5 - Rtin----
      num05 <-
        lulc %>% dplyr::filter(From != To, To == class_n) %>%
        dplyr::group_by(Period, From, Interval) %>% dplyr::summarise(num05 = sum(km2))

      denom05 <-
        lulc %>% dplyr::filter(From != class_n) %>%
      ...
      # EQ3 - Gtj ----
      num03 <- lulc %>% dplyr::filter(From != To) %>%
        dplyr::group_by(Period, To, Interval) %>% dplyr::summarise(num03 = sum(QtPixel)) # gross gain class in time point Yt+1

      denom03 <-
        lulc %>% dplyr::group_by(Period, To) %>% dplyr::summarise(denom03 = sum(QtPixel)) # total area class in time point Yt+1
      ...
      # EQ4 -   Lti ---------
      num04 <- lulc %>% dplyr::filter(From != To) %>%
        dplyr::group_by(Period, From, Interval) %>% dplyr::summarise(num04 = sum(QtPixel)) # gross loss of class i in time point Yt

      denom04 <-
        lulc %>% dplyr::group_by(Period, From) %>% dplyr::summarise(denom04 = sum(QtPixel)) # total area of the class in time point Yt
      ...
      denom05 <-
        lulc %>% dplyr::filter(From != class_n) %>%  # filtering all non-n
